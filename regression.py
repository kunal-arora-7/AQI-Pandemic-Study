# regression.py performs AQI prediction using machine learning models on historical air quality data, utilizing
# a 5-fold cross-validation approach to train and evaluate models on different validation splits across 2000-2018
# AQI and predictor data. Models attempted for training and testing include linear, Ridge, and Lasso (with multiple
# lambda values). The linear model (stronger than Ridge/Lasso in this case) is then analyzed to determine relative
# feature importance of predictors and tested on future test years (2019, 2020, and 2024). R^2, MAE, and RMSE are
# calculated for the linear model on the training, validation, and test sets, and similarly, plots are generated on
# predicted AQI vs. actual AQI for the training, validation, and test sets.
